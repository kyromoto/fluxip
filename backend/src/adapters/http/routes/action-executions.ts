...
export function createActionExecutionsRoutes(deps: ActionExecutionsRouteDeps): Hono {
  ...
  router.get("/actions/:actionId/executions", async (c) => {
    ...
    // Not found (rather than 403) if it isn't this tenant's Action — avoids confirming existence (FR-013/SC-003).
    if (!state.actionId || state.accountId !== auth.tenantId) {
      return c.json({ error: "not_found" }, 404);
    }

    const items = await listExecutionsProjection(deps.redis, deps.eventStore, auth.tenantId, actionId);
    return c.json({ items });
  });
