 ...
 * routed content when the current tenant hasn't completed it yet (FR-018),
 * otherwise renders children unchanged.
 */
export function OnboardingGate(props: { children?: JSX.Element }) {
  const [tenantId] = createResource(fetchTenantId);
  ...
  return (
    <Show when={!tenantId.loading} fallback={null}>
      <Show when={shouldShowOnboarding()} fallback={props.children}>
        <OnboardingWizard
          onComplete={() => {
            markOnboardingComplete(tenantId() as string);
