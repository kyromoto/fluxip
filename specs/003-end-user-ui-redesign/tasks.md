...
- [X] T017 [P] [US1] Create the per-account `localStorage` onboarding-progress helper in `frontend/src/lib/onboarding-state.ts` per research.md §2 (`fluxip.onboarding.<accountId>.completed`)
...
- [X] T022 [US1] Gate onboarding in `frontend/src/App.tsx` (or `AppShell.tsx`): render `OnboardingWizard` instead of normal routed content when authenticated and the onboarding-state flag is absent for the current account (depends on T016, T021) — implemented as `OnboardingGate.tsx`, fetches `GET /account` for the account id and wraps `AppShell`'s authenticated branch
