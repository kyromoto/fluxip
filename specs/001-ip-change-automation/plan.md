...
`.specify/memory/constitution.md` is now ratified (v1.0.0, 2026-07-23): its sole principle, Explicit Commit Authorization, governs commit workflow, not architecture — it imposes no gate on this plan's technical decisions. The architecture guardrails supplied for this feature (pragmatic hexagonal architecture, no separation without a real second adapter, event sourcing with Postgres as the store of record, projections as disposable Redis read models that never back business decisions) remain this plan's de facto constraints, reflected in the Project Structure and research.md below, since the constitution does not (yet) codify them.

**Post-Phase-1 re-check**: `data-model.md` and `contracts/` were reviewed against the same guardrails — `ports/` only exists for the event store and Action/notification-channel adapters (the two places research.md identifies a real, imminent second implementation); no port was introduced for internal-only logic (e.g. debounce scheduling, credential hashing) that has no second adapter in sight. No constitution gates violated. No new complexity requiring justification in Complexity Tracking below.

**2026-07-24 re-check (FR-035, Hetzner Cloud API only)**: Re-evaluated after adding FR-035 (spec.md) and research.md §18 (Hetzner Cloud API `zones`/`rrsets`, resolving the DNS-Update Action's actual API host/resource model). This is a same-adapter, same-port change — the `ActionExecutor` port and `adapters/actions/hetzner-dns/` seam are unaffected; only that adapter's internal HTTP calls and its Provider Credential's expected token type change. No new port, no new complexity, no constitution gate implicated. Still committed only against v1.0.0's commit-authorization principle.
