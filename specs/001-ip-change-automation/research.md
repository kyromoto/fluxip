...
- **Decision**: Use the `cloudevents` npm SDK to construct/validate event envelopes. `source` is read once at startup from `BACKEND_CLOUDEVENTS_SOURCE`; `type` is assembled as `${BACKEND_CLOUDEVENTS_TYPE_PREFIX}.<aggregate>.<event>` (e.g. `space.kyro.fluxip.ip_client.ip_changed`) from `BACKEND_CLOUDEVENTS_TYPE_PREFIX`. The event's business payload (aggregate-specific fields) lives in CloudEvents `data`; `id` is a generated ULID; `time` is set at append time.
...
- **Decision**: BullMQ job-level retry with exponential backoff: 5 attempts, base delay 30s, doubling (30s, 60s, 120s, 240s, 480s ≈ 15 minutes total), after which the execution is recorded as a final failure (`action_execution.failed` with `retriesExhausted: true`). Values are read from env (`BACKEND_ACTION_RETRY_ATTEMPTS`, `BACKEND_ACTION_RETRY_BASE_DELAY_MS`) so they can be tuned per deployment without a code change.
...
## 18. Hetzner integration: Cloud API only, `set_records` action (resolves FR-035)

- **Decision**: The `hetzner-dns` `ActionExecutor` adapter MUST call the Hetzner **Cloud API** (`https://api.hetzner.cloud/v1`) exclusively, using the manually verified, working endpoint: `POST /v1/zones/{zone}/rrsets/{name}/{type}/actions/set_records` (e.g. `.../zones/kyromoto.de/rrsets/@/A/actions/set_records`), body `{ "records": [{ "value": "<ip>", "comment": "<BACKEND_CLOUDEVENTS_SOURCE without protocol> | <execution timestamp, ISO-8601>" }] }`, Bearer auth with a Cloud API token stored as the Action's Provider Credential. `{zone}` and `{name}` are used directly (not percent-encoded, including the literal `@` zone-apex token) since DNS zone/record names only ever contain characters already valid unencoded in a URL path segment. No separate zone-ID-resolution or rrset-listing call is needed or made — one POST per address family, per execution.
- **Rationale**: Hetzner has migrated DNS zone/record management into the Cloud API, issued under new Cloud API tokens — tokens created in the old, separate DNS Console/API are explicitly documented as not working against the Cloud API. This confirms FR-035's "no fallback, ever" clarification is actually satisfiable: the DNS-Update Action's full existing functionality (update an existing A/AAAA record) is available via the Cloud API today, so the Edge Case's "capability not available" branch does not currently apply to FR-008. `set_records` is a per-rrset action scoped to an already-existing rrset resource (Hetzner's action-on-a-resource convention, as with e.g. server actions) and is expected to fail rather than create when the target rrset doesn't exist, so it satisfies FR-008 ("creating new records is not supported") without a separate pre-flight existence check — a failure there is still surfaced diagnosably via the adapter's existing non-2xx/non-JSON error handling. An earlier revision of this section (and of the shipped adapter) called the generic `zones`/`rrsets` collection endpoints with a zone-ID-resolution step instead; that shape did not actually work end-to-end and was replaced by this manually verified request after live testing surfaced the mismatch.
- **Alternatives considered**: Keep calling the legacy/consolidated `api.hetzner.com/v1/dns` host — rejected outright by FR-035, and doubly moot now that it requires DNS-Console-issued tokens FluxIP's Provider Credential model no longer accepts (Assumptions: "Hetzner API token" ≡ Cloud API token). A DNS-API fallback for any gap — rejected per FR-035's Session 2026-07-24 clarification (no fallback under any circumstance); moot here since no gap exists for the current Action's required capability. Resolving the zone name to a numeric zone ID via a separate `GET /zones?name=` lookup, then listing rrsets to find a match — rejected in favor of the simpler, manually verified direct `set_records` call, which needs neither call.
